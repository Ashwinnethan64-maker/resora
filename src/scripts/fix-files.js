...
// 1. Fix src/lib/resources/resolve-target.ts
const resolveTargetPath = path.join(process.cwd(), 'src', 'lib', 'resources', 'resolve-target.ts');
const resolveTargetCode = `import { ResourceModel } from '@/types/database';
...
`;
fs.writeFileSync(resolveTargetPath, resolveTargetCode, 'utf8');
console.log('Fixed resolve-target.ts');

// 2. Fix src/components/ui/MarkdownRenderer.tsx
const markdownRendererPath = path.join(process.cwd(), 'src', 'components', 'ui', 'MarkdownRenderer.tsx');
const markdownRendererCode = `'use client';
...
`;
fs.writeFileSync(markdownRendererPath, markdownRendererCode, 'utf8');
